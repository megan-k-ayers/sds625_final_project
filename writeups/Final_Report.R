...
#' information from multiple data sources. To attempt to understand climate
#' change beliefs, which I will consider to be the response variable, I
#' will use data collected and processed by the Yale Program on Climate Change
#' Communication. This data consists of US county-level estimates of climate
#' change beliefs, i.e. x% of the population in New Haven County believes that
#' climate change is happening. These estimates are calculated from a
#' large-scale nation-wide survey (n > 25,000) conducted in 2020, and were
#' "derived from a statistical model using multilevel regression with
#' post-stratification (MRP)" While individual-level data may be more powerful
#' for arguing existence of a causal relationship between extreme weather
#' event experiences and climate change beliefs, actually obtaining the
#' supplementary data at this level was not feasible for this project due to
#' data privacy issues and the fact that it is difficult to guarantee that an
